...
export const AboutStory = () => {
  ...
  return (
    <section ref={containerRef} className="h-[300vh] relative bg-light z-20" style={{ overflow: 'clip' }}>
      ...
      <div className="sticky top-0 h-screen w-full flex flex-col md:flex-row overflow-x-hidden max-w-[1600px] mx-auto px-6 lg:px-24 pt-20 md:pt-24">

        {/* Left Side: Pinned Text Story */}
        <div className="w-full md:w-1/2 h-full flex flex-col justify-start pt-4 md:pt-16 pr-0 md:pr-12 relative z-10 overflow-y-auto no-scrollbar pb-8">
            ...
            {storyLines.map((line, i) => {
              ...
              return (
                <motion.h3
                  key={i}
                  style={{ opacity }}
                  className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-bold tracking-tighter leading-[1.1] text-black"
                >
                  {line}
                </motion.h3>
              );
            ...
            {skillStatements.map((skill, i) => {
              ...
              return (
                <motion.div
                  key={i}
                  style={{ opacity, x }}
                  className="flex items-start gap-3 md:gap-4 text-sm sm:text-base md:text-2xl font-light text-gray-700"
                >
                  <div className="w-2 h-2 rounded-full bg-black shrink-0 mt-2 md:mt-3.5" />
                  <p>{skill}</p>
                </motion.div>
              );
            })}
